...
module.exports = function toggleCodeSnippet(options) {
	...
	for (var i = 0; i < demoHeadings.length; i++) {
		...
		demoHeadings[i].querySelector(opts.showCodeBtnSelector).addEventListener('click', function (ev) {
			ev.preventDefault();

			nextItem(this.parentNode, opts.codeContainerEl, function (el) {
				el.classList.toggle(opts.codeContainerEl);
...
var defaultOptions = {
	...
	showCodeBtnSelector: '.sg-demoHeading-showCodeBtn',
	demoContainerEl: '.sg-demoHeading-showCodeBtn',
